...
- Format with **bold** for emphasis and bullet points for lists.`;
  ...
  if (displayMode === 'compact') {
    prompt += `\n- Keep responses SHORT (max 3-4 bullet points). Use very brief bullets — no pipes or long stat lines.`;
    prompt += `\n- When comparing players: name the top pick + one-line reason. A data table is shown alongside your text, so do NOT repeat raw stats. Focus on the takeaway.`;
    prompt += `\n- Format example: "**Robinson** is your clear starter — elite clutch with solid culture fit. **Ramirez** is the ideal backup."`;
  } else if (displayMode === 'half') {
    prompt += `\n- Provide moderate detail: 2-4 short paragraphs with key context. A data table may appear alongside, so avoid repeating exact stat values in text — focus on insights and recommendations.`;
  } else if (displayMode === 'full') {
    prompt += `\n- Provide comprehensive analysis. Use ## headers to organize. Include all relevant data points.`;
  }
...
function generateQuickReplies({ playerData, mentionedPlayers, viewContext, chunks, assistantMessage }) {
  ...
  // Multiple player matches — offer each as an option
  if (!playerData && mentionedPlayers && mentionedPlayers.length > 1) {
...
function generateVisualizations({ playerData, mentionedPlayers, visiblePlayers, viewContext, assistantMessage, userMessage }) {
  ...
  // 1. Comparison Table — 2-3 mentioned players
  if (mentionedPlayers && mentionedPlayers.length >= 2 && mentionedPlayers.length <= 3) {
    const tablePlayers = mentionedPlayers.slice(0, 3).map(p => buildPlayerVizData(p, visiblePlayers));
    // Compute insight
    let insight = '';
