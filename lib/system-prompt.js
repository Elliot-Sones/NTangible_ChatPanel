...
function formatPlayerCompact(p) {
  ...
  let line = '';
  line += `- Position: ${p.position || 'N/A'} | Level: ${p.level || 'N/A'} | Type: ${p.type || 'N/A'}\n`;
  line += `- Clutch Factor: ${p.clutch_factor || 'N/A'} (${ext.scoring_range || 'N/A'}) | Fit Score: ${p.fit_score || 'N/A'}% (${ext.fit_label || 'N/A'}) | Cohort: ${ext.archetype_category || 'N/A'}\n`;
  line += `- Communication: ${p.comm_style || 'N/A'} | Learning: ${p.learning_style || 'N/A'} | Motivation: ${p.motivation || 'N/A'}\n`;
  return line;
}
...
- Format with **bold** for emphasis and bullet points for lists.`;
  ...
  if (displayMode === 'compact') {
    prompt += `\n- Keep responses to 2-3 sentences max. Bullet points only. No headers.`;
  } else if (displayMode === 'half') {
    prompt += `\n- Provide moderate detail: 2-4 short paragraphs with key context.`;
